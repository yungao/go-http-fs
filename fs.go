...
const SEND_BLOCK_SIZE = 128 * 1024 // 128KB
...
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker, callbakFunc func(start, send int64)) {
	...
	if r.Method != "HEAD" {
		if callbakFunc != nil {
			var index = 0
			var length int64
			var block = make([]byte, SEND_BLOCK_SIZE)
			for {
				var left = sendSize - length
				if left <= 0 {
					break
				}
				if left < SEND_BLOCK_SIZE {
					block = make([]byte, left)
				}

				nr, err := sendContent.Read(block)
				if err != nil {
					if err != io.EOF {
						break
					} else {
				...
				nw, err := w.Write(block[:nr])
				if err != nil || nr != nw {
					break
				}

				length += int64(nw)
				if index++; index%8 == 0 { // callback every 1MB
					callbakFunc(startPos, length)
					index = 0
				}
			}
...
func checkPreconditions(w http.ResponseWriter, r *http.Request, modtime time.Time) (done bool, rangeHeader string) {
	...
	rangeHeader = getHeader(r.Header, "Range")
	if rangeHeader != "" {
...
func serveFile(w http.ResponseWriter, r *http.Request, fs FileSystem, name string, redirect bool, callbakFunc func(start, send int64)) {
	...
	serveContent(w, r, d.Name(), d.ModTime(), sizeFunc, f, callbakFunc)
}
...
// use ServeContent.
func ServeFile(w http.ResponseWriter, r *http.Request, name string, callbakFunc func(start, send int64)) {
	...
	serveFile(w, r, Dir(dir), file, false, callbakFunc)
}
...
func (f *fileHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	serveFile(w, r, f.root, path.Clean(upath), true, nil)
}
